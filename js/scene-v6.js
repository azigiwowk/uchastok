...
function updatePaths3D(){groups.site.children.filter(m=>m.userData.pathId).forEach(m=>{const p=LAYOUT_PATHS.find(p=>p.id===m.userData.pathId);if(!p)return;const dx=p.b[0]-p.a[0],dz=p.b[1]-p.a[1];if(m.userData.v6PathGeometry)m.geometry.dispose();m.geometry=new THREE.BoxGeometry(p.width,.05,Math.hypot(dx,dz));m.userData.v6PathGeometry=true;m.position.set((p.a[0]+p.b[0])/2,.025,(p.a[1]+p.b[1])/2);m.rotation.y=Math.atan2(dx,dz);});}
let houseDoor=null;
