...
function createBernRenderer(options = {}) {
  ...
  BernV6.rendererMode = "2d-fallback";
  const canvas = document.createElement("canvas");
  canvas.setAttribute("aria-label", "2D-план участка — WebGL недоступен");
  const fallback = {
    domElement: canvas,
    shadowMap: {},
    ...
    setPixelRatio() {},
    render() {
      if (!BernV6.drawPlan) return;
      const source = BernV6.drawPlan(),
      ...
      c.fillText("WebGL недоступен · резервный 2D-вид", 20, h - 55);
    },
  };
  return fallback;
}
